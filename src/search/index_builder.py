...
def load_corpus(corpus_path: str):
    ...
    build_data = []
    if "2wikimultihopqa" in corpus_path:
            train = json.load(open(os.path.join(corpus_path, "train.json"), "r"))
            dev = json.load(open(os.path.join(corpus_path, "dev.json"), "r"))
            test = json.load(open(os.path.join(corpus_path, "test.json"), "r"))
    ...
    elif "hotpotqa" in corpus_path:
        from multiprocessing import Pool
        filelist = glob(os.path.join(corpus_path, "*/wiki_*.bz2"))
    ...
    elif "musique" in corpus_path:
        train = [
            json.loads(line.strip())
            for line in open(os.path.join(corpus_path, "musique_ans_v1.0_train.jsonl"))
        ] + [
            json.loads(line.strip())
            for line in open(os.path.join(corpus_path, "musique_full_v1.0_train.jsonl"))
        ]
        dev = [
            json.loads(line.strip())
            for line in open(os.path.join(corpus_path, "musique_ans_v1.0_dev.jsonl"))
        ] + [
            json.loads(line.strip())
            for line in open(os.path.join(corpus_path, "musique_full_v1.0_dev.jsonl"))
        ]
        test = [
            json.loads(line.strip())
            for line in open(os.path.join(corpus_path, "musique_ans_v1.0_test.jsonl"))
        ] + [
            json.loads(line.strip())
            for line in open(os.path.join(corpus_path, "musique_full_v1.0_test.jsonl"))
        ]
        ...
        corpus = datasets.load_dataset(
            'json', 
            data_files=f"{corpus_path}/*.jsonl",
            split="train",
            num_proc=4)
        print("corpus num",corpus)
        return corpus
    build_data = [{"id":item["id"], "contents":'"' + item["title"] + '"\n' + item["text"]} for item in build_data]
    corpus = datasets.Dataset.from_list(build_data)
